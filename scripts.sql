...
CREATE TABLE IF NOT EXISTS `mydb`.`curso` (
  ...
  `ativo` VARCHAR(1) NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_habilidades_cursos_idx` (`habilidade_id` ASC) VISIBLE,
  INDEX `fk_receitas_cursos_idx` (`receita_id` ASC) VISIBLE,
  CONSTRAINT `fk_habilidades_cursos`
    FOREIGN KEY (`habilidade_id`)
    REFERENCES `mydb`.`habilidade` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_receitas_cursos`
    FOREIGN KEY (`receita_id`)
    REFERENCES `mydb`.`receita` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
